...
export class MixScene extends Scene{
    ...
    private setItemList(){
        ...
        Item.values()
            .forEach(item=>{
                if(!item.mix || !item.mix.isVisible()){return;}
                const mix = item.mix;
                const color = ()=>{
                    if(item === this.choosedObj){return Color.CYAN;}
                    if(!mix.canRun())           {return Color.GRAY;}
                    return Color.WHITE;
                };
                this.list.add({
    ...
    private setEqList(){
        ...
        Eq.values()
            .forEach(eq=>{
                if(!eq.mix || !eq.mix.isVisible()){return;}
                const mix = eq.mix;
                const color = ()=>{
                    if(eq === this.choosedObj){return Color.CYAN;}
                    if(!mix.canRun())           {return Color.GRAY;}
                    return Color.WHITE;
                };
                this.list.add({
